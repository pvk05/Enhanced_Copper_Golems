package com.philipvk.enhanced_copper_golems.entities.ai;
...
public class SmartMoveItemsTask extends MultiTickTask<PathAwareEntity> {
    ...
    private final Predicate<SmartMoveItemsTask.Storage> storagePredicate;
    private final Consumer<PathAwareEntity> travellingCallback;
    private final Map<SmartMoveItemsTask.InteractionState, SmartMoveItemsTask.InteractionCallback> interactionCallbacks;
    @Nullable
    private SmartMoveItemsTask.Storage targetStorage = null;
    private SmartMoveItemsTask.NavigationState navigationState;
    @Nullable
    private SmartMoveItemsTask.InteractionState interactionState;
    private int interactionTicks;

    public SmartMoveItemsTask(
            float speed,
            Predicate<BlockState> inputContainerPredicate,
            Predicate<BlockState> outputChestPredicate,
            int horizontalRange,
            int verticalRange,
            Map<SmartMoveItemsTask.InteractionState, SmartMoveItemsTask.InteractionCallback> interactionCallbacks,
            Consumer<PathAwareEntity> travellingCallback,
            Predicate<SmartMoveItemsTask.Storage> storagePredicate
    ) {
        super(
                ImmutableMap.of(
        ...
        this.navigationState = SmartMoveItemsTask.NavigationState.TRAVELLING;
    }
    ...
    protected void keepRunning(ServerWorld serverWorld, PathAwareEntity pathAwareEntity, long l) {
        ...
        if (this.targetStorage == null) {
            ...
            if (this.navigationState.equals(SmartMoveItemsTask.NavigationState.QUEUING)) {
                this.tickQueuing(this.targetStorage, serverWorld, pathAwareEntity);
            }

            if (this.navigationState.equals(SmartMoveItemsTask.NavigationState.TRAVELLING)) {
                this.tickTravelling(this.targetStorage, serverWorld, pathAwareEntity);
            }

            if (this.navigationState.equals(SmartMoveItemsTask.NavigationState.INTERACTING)) {
    ...
    private boolean tick(ServerWorld world, PathAwareEntity entity) {
        if (!this.hasValidTargetStorage(world, entity)) {
            ...
            Optional<SmartMoveItemsTask.Storage> optional = this.findStorage(world, entity);
            if (optional.isPresent()) {
                this.targetStorage = (SmartMoveItemsTask.Storage)optional.get();
    ...
    private void tickQueuing(SmartMoveItemsTask.Storage storage, World world, PathAwareEntity entity) {
        if (!this.matchesStoragePredicate(storage, world)) {
            this.onCannotUseStorage(entity);
        }
    }

    protected void tickTravelling(SmartMoveItemsTask.Storage storage, World world, PathAwareEntity entity) {
    ...
    protected void tickInteracting(SmartMoveItemsTask.Storage storage, World world, PathAwareEntity entity) {
    ...
    private void transitionToQueuing(PathAwareEntity entity) {
        ...
        this.setNavigationState(SmartMoveItemsTask.NavigationState.QUEUING);
    }

    private void onCannotUseStorage(PathAwareEntity entity) {
        this.setNavigationState(SmartMoveItemsTask.NavigationState.TRAVELLING);
        this.walkTowardsTargetStorage(entity);
    }
    ...
    private void transitionToInteracting(SmartMoveItemsTask.Storage storage, PathAwareEntity entity) {
        this.selectInteractionState(
                entity,
                storage.inventory,
                this.createSetInteractionStateCallback(SmartMoveItemsTask.InteractionState.PICKUP_ITEM),
                this.createSetInteractionStateCallback(SmartMoveItemsTask.InteractionState.PICKUP_NO_ITEM),
                this.createSetInteractionStateCallback(SmartMoveItemsTask.InteractionState.PLACE_ITEM),
                this.createSetInteractionStateCallback(SmartMoveItemsTask.InteractionState.PLACE_NO_ITEM)
        );
        this.setNavigationState(SmartMoveItemsTask.NavigationState.INTERACTING);
    }
    ...
    private BiConsumer<PathAwareEntity, Inventory> createSetInteractionStateCallback(SmartMoveItemsTask.InteractionState state) {
        return (entity, inventory) -> this.setInteractionState(state);
    }

    private void setNavigationState(SmartMoveItemsTask.NavigationState state) {
        this.navigationState = state;
    }

    private void setInteractionState(SmartMoveItemsTask.InteractionState state) {
        this.interactionState = state;
    }

    private void setLookTarget(SmartMoveItemsTask.Storage storage, PathAwareEntity entity) {
        entity.getBrain().remember(MemoryModuleType.LOOK_TARGET, new BlockPosLookTarget(storage.pos));
        this.resetNavigation(entity);
        if (this.interactionState != null) {
            Optional.ofNullable((SmartMoveItemsTask.InteractionCallback)this.interactionCallbacks.get(this.interactionState))
    ...
    private Optional<SmartMoveItemsTask.Storage> findStorage(ServerWorld world, PathAwareEntity entity) {
        ...
        SmartMoveItemsTask.Storage storage = null;
        ...
        for (ChunkPos chunkPos : list) {
            ...
            if (worldChunk != null) {
                for (BlockEntity blockEntity : worldChunk.getBlockEntities().values()) {
                    if (isValidContainer(blockEntity)) {
                        double e = blockEntity.getPos().getSquaredDistance(entity.getEntityPos());
                        if (e < d) {
                            SmartMoveItemsTask.Storage storage2 = this.getStorageFor(entity, world, blockEntity, set, set2, box);
                            if (storage2 != null) {
                                storage = storage2;
                                d = e;
                            }
    ...
    private boolean isValidContainer(BlockEntity be) {
        return (be instanceof ChestBlockEntity && Enhanced_copper_golems.CONFIG.chest() && !(be instanceof TrappedChestBlockEntity))
                || (be instanceof TrappedChestBlockEntity && Enhanced_copper_golems.CONFIG.trappedChest())
                || (be instanceof BarrelBlockEntity && Enhanced_copper_golems.CONFIG.barrel())
                || (be instanceof ShulkerBoxBlockEntity && Enhanced_copper_golems.CONFIG.shulkerbox());
    }

    @Nullable
    private SmartMoveItemsTask.Storage getStorageFor(
        ...
        if (!bl) {
            ...
            SmartMoveItemsTask.Storage storage = SmartMoveItemsTask.Storage.forContainer(blockEntity, world);
            if (storage == null) {
                return null;
            } else {
    ...
    private boolean isLocked(SmartMoveItemsTask.Storage storage) {
    ...
    private boolean hasValidTargetStorage(World world, PathAwareEntity entity) {
        ...
        if (bl && !this.isContainerBlocked(world, this.targetStorage)) {
            if (!this.navigationState.equals(SmartMoveItemsTask.NavigationState.TRAVELLING)) {
                return true;
            }
    ...
    private boolean canNavigateTo(World world, SmartMoveItemsTask.Storage storage, PathAwareEntity entity) {
    ...
    private boolean isContainerBlocked(World world, SmartMoveItemsTask.Storage storage) {
        if (storage.blockEntity instanceof ChestBlockEntity) {
            return ChestBlock.isChestBlocked(world, storage.pos);
        }
        if (storage.blockEntity instanceof ShulkerBoxBlockEntity) {
            if (((ShulkerBoxBlockEntity) storage.blockEntity).getAnimationStage() != ShulkerBoxBlockEntity.AnimationStage.CLOSED) {
                return false;
            } else {
                Box box = ShulkerEntity.calculateBoundingBox(1.0F, storage.state.get(ShulkerBoxBlock.FACING), 0.0F, 0.5F, storage.pos.toBottomCenterPos()).contract(1.0E-6);
    ...
    private boolean isUnchanged(World world, SmartMoveItemsTask.Storage storage) {
        return storage.blockEntity.equals(world.getBlockEntity(storage.pos));
    }

    private Stream<SmartMoveItemsTask.Storage> getContainerStorages(SmartMoveItemsTask.Storage storage, World world) {
        if (storage.state.get(ChestBlock.CHEST_TYPE, ChestType.SINGLE) != ChestType.SINGLE) {
            SmartMoveItemsTask.Storage storage2 = SmartMoveItemsTask.Storage.forContainer(ChestBlock.getPosInFrontOf(storage.pos, storage.state), world);
    ...
    private boolean hasVisited(Set<GlobalPos> visitedPositions, Set<GlobalPos> checkedPositions, SmartMoveItemsTask.Storage storage, World visited) {
    ...
    private boolean isWithinRange(double range, SmartMoveItemsTask.Storage storage, World world, PathAwareEntity entity, Vec3d pos) {
    ...
    private boolean isVisible(World world, boolean nextToStorage, Vec3d pos, SmartMoveItemsTask.Storage storage, PathAwareEntity entity) {
        return nextToStorage && this.isVisible(storage, world, entity, pos);
    }

    private boolean isVisible(SmartMoveItemsTask.Storage storage, World world, PathAwareEntity entity, Vec3d pos) {
    ...
    private boolean matchesStoragePredicate(SmartMoveItemsTask.Storage storage, World world) {
    ...
    @FunctionalInterface
    public interface InteractionCallback extends TriConsumer<PathAwareEntity, SmartMoveItemsTask.Storage, Integer> {
    }
    ...
    public record Storage(BlockPos pos, Inventory inventory, BlockEntity blockEntity, BlockState state) {
        ...
        @Nullable
        public static SmartMoveItemsTask.Storage forContainer(BlockEntity blockEntity, World world) {
            BlockPos blockPos = blockEntity.getPos();
            BlockState blockState = blockEntity.getCachedState();
            Inventory inventory = getInventory(blockEntity, blockState, world, blockPos);
            return inventory != null ? new SmartMoveItemsTask.Storage(blockPos, inventory, blockEntity, blockState) : null;
        }

        @Nullable
        public static SmartMoveItemsTask.Storage forContainer(BlockPos pos, World world) {
